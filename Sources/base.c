...
unsigned char make_lineX(int x1, int x2) {
  ...
  //x2 = 4;
  //x = 0xFE & ~(0xFB)
  //    0x11111110 & ~(0x11110000)
  //    0x11111110 &  (0x00001111)
  //           0x00001110
  x = (0xFF << x1) & ~(0xFF << (x2+1));
...
void shiftCubeNoWrapAround(char axis, int units)
{
  ...
  if (axis == 'x')
  {
    if(units == 1)
    {
      for(shift_layer=0;shift_layer<8;shift_layer++)
      {
        ...
        {
          //one_bit_temp = cube[shift_layer][shift_row] & 0x01;
    ...
    else if (units == -1)
    {
      for(shift_layer=0;shift_layer<8;shift_layer++)
      {
        for(shift_row=0;shift_row<8;shift_row++)
        {
          //one_bit_temp = cube[shift_layer][shift_row] & 0x80;
          cube[shift_layer][shift_row] <<= 1;
  ...
  else if (axis == 'y')
  {
    if(units == 1)
    {
      for(shift_layer=0;shift_layer<8;shift_layer++)
      {
        //one_char_temp = cube[shift_layer][0];
        shift_index = 7;
