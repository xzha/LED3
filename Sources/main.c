...
void effect_pick(void) {
  if (effect_count == 0) {
    ...
    for (i=0;i<8 && effect_count == 2;i++){
      effect_box_shrink_grow (1, i%4, i & 0x04, 50);
	  }
  } else if (effect_count == 3) {
	  effect_box_center(50,0);
		effect_box_center(50,1);
		effect_box_center(50,0);
		effect_box_center(50,1);
  } else if (effect_count == 4) {
    effect_path_text (120, "MERRY XMAS");
  } else if (effect_count == 5) {
    if (effect_count == 5) {
      effect_holupp('z',1,500);
      delay_ms(400);
    }
    if (effect_count == 5) {
      effect_holupp('z',-1,500);
